...
(async () => {
  ...
  try {
    ...
    await page.locator('input[name="wizardDatasetMode"][value="flexible_shared"]').check();
    ...
    assert.equal(await page.locator('.chartColumnSeriesChoice[data-column-id="0"]').count(), 1);
    ...
    assert.equal(await page.locator('#chartColumnCategoryLabel').innerText(), 'Timestamp column');
    ...
    assert.equal(await page.locator('#chartColumnMappingAutomatic').isChecked(), true);
    await page.locator('#chartColumnMappingAutomatic').uncheck();
    await page.locator(`.chartColumnSeriesChoice[data-column-id="${refs.Product}"]`).uncheck();
    await page.locator(`.chartColumnValueChoice[data-column-id="${refs.Cost}"]`).uncheck();
    await page.locator('#analyticsDialogBtnGo').click();
