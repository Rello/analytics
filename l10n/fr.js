OC.L10N.register(
    "analytics",
    {
    "Dataset" : "Jeu de données",
    "Data" : "Donnée",
    "Dataload" : "Chargement de données",
    "Data maintenance is not possible for this type of report" : "La gestion des données n'est pas possible pour ce type de rapport",
    "This report does not have any dataloads. <br>Choose a datasource from the dropdown and press \"+\"" : "Ce rapport ne contient aucun chargement de données. <br>Choisissez une source de données dans la liste déroulante et appuyez sur \"+\"",
    "Choose a dataload from the list to change its settings" : "Choisissez un chargement de données dans la liste pour modifier ses paramètres",
    "Saved" : "Enregistré",
    "Are you sure?" : "Êtes-vous sûr ?",
    "Delete dataload" : "Supprimer le chargement de données",
    "Select file" : "Sélectionner un fichier",
    "Datasource simulation" : "Simulation de source de données",
    "Load started" : "Chargement démarré",
    "records inserted" : "enregistrements ajoutés",
    "records updated" : "enregistrements mis à jour",
    "errors" : "erreurs",
    "No maintenance possible" : "Aucune gestion possible",
    "This dataset is not used!" : "Cet jeu de données n'est pas utilisé!",
    "All data including all reports will be deleted!" : "Tous les rapports vont être supprimés!",
    "Delete" : "Supprimer",
    "Records to be deleted: " : "Enregistrement à supprimer : ",
    "Delete data" : "Supprimer la donnée",
    "Technical error. Please check the logs." : "Erreur technique. Merci de vérifier les logs.",
    "Search" : "Recherche",
    "Show _MENU_ entries" : "Afficher les entrées _MENU_",
    "Showing _START_ to _END_ of _TOTAL_ entries" : "Affichage des entrées _START_ to _END_ of _TOTAL_ ",
    "Showing 0 to 0 of 0 entries" : "Affichage de 0 à 0 sur 0 entrées",
    "first" : "premier",
    "previous" : "précédent",
    "next" : "suivant",
    "last" : "dernier",
    "Please select" : "Veuillez sélectionner",
    "equal to" : "égal à",
    "greater than" : "plus grand que",
    "less than" : "plus petit que",
    "contains" : "contient",
    "list of values" : "liste de valeurs",
    "Drilldown" : "Exploration",
    "OK" : "OK",
    "Cancel" : "Annuler",
    "Filter" : "Filtrer",
    "Filter by" : "Filtrer par",
    "Operator" : "Opérateur ",
    "Value" : "Valeur",
    "and" : "et",
    "Add" : "Ajouter",
    "Primary" : "Primaire",
    "Secondary" : "Secondaire",
    "Line" : "Ligne",
    "Bar" : "Barre",
    "Chart options" : "Paramètres du graphique",
    "Data series" : "Séries de données",
    "Vertical axis" : "Axe vertical (ordonnée)",
    "Chart type" : "Type de graphique",
    "Color" : "Couleur",
    "New dataset" : "Nouveau jeu de données",
    "New report" : "Nouveau rapport",
    "Dataset maintenance" : "Gestion des jeu de données",
    "Back to reports" : "Retour au rapports",
    "Overview" : "Vue d'ensemble",
    "Scheduled dataload" : "Chargement de données planifié",
    "Basic settings" : "Paramètres de base",
    "Remove from favorites" : "Retirer des favoris",
    "Add to favorites" : "Ajouter aux favoris",
    "Delete folder" : "Supprimer le dossier",
    "Report" : "Rapport",
    "Thresholds" : "Seuils",
    "Share" : "Partage",
    "All data will be deleted!" : "Toutes les données seront supprimées !",
    "Text variables can be used in the name or subheader.<br>They are replaced when the report is executed." : "Des variables de texte peuvent être utilisées dans le nom ou le sous-en-tête. <br>Elles sont remplacées lors de l'exécution du rapport.",
    "Text variables" : "Variables texte",
    "Reports can be grouped into a folder structure" : "Les rapports peuvent être regroupés dans un dossier",
    "Do you want to create a new group folder?" : "Voulez-vous créer un nouveau groupement de dossier ?",
    "Report group" : "Groupement de dossiers",
    "Column descriptions are derived from the dataset and can not be changed on a report level" : "Les descriptions de colonnes sont dérivées du jeu de données et ne peuvent pas être modifiées au niveau du rapport",
    "Datasets can be changed in a separate maintenance" : "Les jeux de données peuvent être modifiés dans une gestion séparée",
    "Switch to the dataset maintenance from here?" : "Passer à la gestion du jeu de données à partir d'ici?",
    "New" : "Nouveau",
    "Use this endpoint to submit data via an API:" : "Utilisez ce point de terminaison pour envoyer des données via une API:",
    "Detail instructions at:" : "Instructions détaillées sur:",
    "REST API parameters" : "Paramètres de l'API REST",
    "No shares found" : "Aucun partage trouvé",
    "Add share link" : "Ajouter un lien de partage",
    "Share link" : "Lien de partage",
    "Link copied" : "Lien copié",
    "Column 1" : "Colonne 1",
    "Column 3" : "Colonne 3",
    "New in" : "Nouveau dans",
    "View changelog" : "Voir le journal des modifications",
    "Show in Analytics" : "Afficher dans Analytics",
    "Analytics" : "Analytics",
    "You created a new report: {report}" : "Vous avez créé un nouveau rapport : {report}",
    "You deleted report {report}" : "Vous avez supprimé un rapport : {report}",
    "You shared report {report}" : "Vous avez partagé un rapport : {report}",
    "shared report {report} with you" : "a partagé le rapport {report} avec vous",
    "You have added new data to dataset {report}" : "Vous avez ajouté de nouvelles données au jeu de données {report}",
    "has added new data to dataset {report}" : "a ajouté de nouvelles données au jeu de données {rapport}",
    "You have imported data in dataset {report}" : "Vous avez importé des données dans le jeu de données {rapport}",
    "has imported data in dataset {report}" : "a importé des données dans le jeu de données {rapport}",
    "New data has been added to dataset {report} via API" : "De nouvelles données ont été ajoutées au jeu de données {report} via l'API",
    "New data has been added to dataset {report} via dataload" : "De nouvelles données ont été ajoutées au jeu de données {report} via le chargement de données",
    "<strong>Analytics</strong>: New data was added to a report" : "<strong>Analytics</strong> : De nouvelles données ont été ajoutées à un rapport",
    "<strong>Analytics</strong>: New reports" : "<strong>Analytique</strong> : Nouveaux rapports",
    "Reports" : "s",
    "Local file" : "Fichier local",
    "File" : "Fichier",
    "Sheet" : "Feuille",
    "sheet name" : "nom de la feuille",
    "Cell range" : "Plage de cellules",
    "e.g. A1:C3,A5:C5" : "ex. A1:C3,A5:C5",
    "External file" : "Fichier externe",
    "External URL" : "URL externe",
    "Ignore leading rows" : "Ignorer les lignes d'en-tête",
    "Number of rows" : "Nombre de lignes",
    "Select columns" : "Sélectionner les colonnes",
    "e.g. 1,2,4 or leave empty" : "p.ex. 1,2,4 ou laisser vide",
    "Limit" : "Limite",
    "Timestamp of dataload" : "Horodatage du chargement de données",
    "false/true" : "faux/vrai",
    "JSON" : "JSON",
    "HTTP method" : "Méthode HTTP",
    "GET/POST" : "GET/POST",
    "Authentication" : "Authentification",
    "Object path" : "Chemin de l'objet",
    "true/false" : "vrai/faux",
    "HTML grabber" : "Téléchargeur HTML",
    "valid regex" : "regex valide",
    "Read file and add its data to an existing dataset" : "Lire fichier et ajouter ses données à un jeu de données existant",
    "Announcements" : "Annonces",
    "Report '{report}': {subject} reached the threshold of {rule} {value}" : "Rapport '{report}' : {subject} a atteint le seuil de {rule} {value}",
    "3rd field must be a valid number" : "Le 3è champ doit contenir un nombre valide",
    "No data" : "Aucune donnée",
    "Last field must be a valid number" : "Le dernier champ doit contenir un nombre valide",
    "copy" : "copier",
    "extract / store / visualize" : "extraire / enregistrer / visualiser",
    "Connect data from different sources in one place.\nEnable flexible reports and enjoy seamless Nextcloud integration.\nVisualize and share anything, from financial analysis to IoT logs.\n\n##### Features:\n- Data sources: csv, spreadsheet, API, imports, manual entry\n- 3rd party: GitHub Statistics, website grabber, JSON\n- Register external data sources from other apps\n- Storage: real-time or database\n- Advanced data loads with scheduling\n- Visualization: combined charts, tables, filters\n- Analysis functions: trend\n- Thresholds for alerts and color coding\n- Integration: Files, Activity, Notifications, Flow, Dashboard, Search\n- Sharing: user, groups, link" : "Connectez les données de différentes sources en un seul endroit.\nActivez des rapports flexibles et profitez d'une intégration Nextcloud transparente.\nVisualisez et partagez n'importe quoi, de l'analyse financière aux journaux IoT.\n\n##### Fonctionnalités:\n- Sources de données : csv, tableur, API, imports, saisie manuelle\n- Tierce partie : GitHub Statistics, capture de site Web (grabber), JSON\n- Enregistrez des sources de données externes à partir d'autres applications\n- Stockage : temps réel ou base de données\n- Chargements de données avancés avec planification\n- Visualisation : graphiques combinés, tableaux, filtres\n- Fonctions d'analyse : tendance\n- Seuils d'alertes et codage couleur\n- Intégration : Fichiers, Activité, Notifications, Flux, Tableau de bord, Recherche\n- Partage : utilisateur, groupes, lien",
    "This report is password protected" : "Ce rapport est protégé par mot de passe",
    "The password is wrong. Try again." : "Le mot de passe est incorrect. Veuillez réessayer.",
    "Password" : "Mot de passe",
    "Legend" : "Légende",
    "No data found" : "Aucune donnée trouvée",
    "Favorites" : "Favoris",
    "More Information …" : "Information supplémentaire ...",
    "Analytics - Advanced configuration" : "Analytics - Configuration avancée",
    "This section is used for dataset maintenance and dataload configurations" : "Cette section est utilisée pour la gestion des jeux de données et les configurations des chargement de données",
    "Please select a dataset" : "Veuillez sélectionner un jeu de données",
    "Options" : "Options",
    "Analysis" : "Analyse",
    "Auto refresh" : "Actualiser automatiquement",
    "Download chart" : "Télécharger le graphique",
    "Report was changed - Press here to save the current state" : "Le rapport a été modifié - Appuyez ici pour enregistrer l'état actuel",
    "Save changes" : "Enregistrer les modifications",
    "Save as new report" : "Enregistrer en tant que nouveau rapport",
    "back" : "retour",
    "Trend" : "Tendance",
    "Linear Regression" : "Régression linéaire",
    "none" : "aucun",
    "1 min" : "1 min.",
    "10 min" : "10 min.",
    "30 min" : "30 min.",
    "New Report Wizard" : "Assistant Nouveau Rapport",
    "Choose a name for the report" : "Choisir un nom pour le rapport",
    "Subheader" : "Sous-titre",
    "Variables" : "Variables",
    "Datasource" : "Source de données",
    "Saved Data" : "Donnée enregistrée",
    "Datasource options" : "Options de source de données",
    "Column headers" : "En-têtes de colonne",
    "Column" : "Colonne",
    "Visualization" : "Visualisation",
    "Display" : "Affichage",
    "Chart" : "Graphique",
    "Table" : "Tableau",
    "Timeline (date in column 2)" : "Chronologie (date dans la colonne 2)",
    "Area" : "Zone",
    "Doughnut" : "Camembert",
    "advanced" : "avancé",
    "Open documentation" : "Ouvrir la documentation",
    "Data options" : "Options de données",
    "Update" : "Mettre à jour",
    "Export" : "Exporter",
    "Settings" : "Paramètres",
    "Import report" : "Importer un rapport",
    "Do you like this app?" : "Aimez-vous cette application ?",
    "About" : "À propos",
    "More information …" : "Information supplémentaire …",
    "Name" : "Nom",
    "Number of records" : "Nombre d'enregistrements",
    "Manual entry" : "Entrée manuelle",
    "Object" : "Objet",
    "Date" : "Date",
    "Save data" : "Enregistrer la donnée",
    "Import" : "Importer",
    "From file" : "Depuis un fichier",
    "From clipboard" : "Depuis le presse-papier",
    "REST API" : "API REST",
    "Advanced configuration" : "Configuration avancée",
    "Thresholds can trigger notifications and color coding in reports" : "Les seuils peuvent déclencher des notifications et un codage couleur dans les rapports",
    "single value or *" : "valeur unique ou *",
    "= equal" : "= égal",
    "> greater" : "> plus grand",
    "< less" : "< plus petit",
    "<= less equal" : "<= plus petit ou égal",
    ">= greater equal" : ">= plus grand ou égal",
    "!= not equal" : "!= pas égal",
    "Severity" : "Gravité",
    "Notification" : "Notification",
    "Red" : "Rouge",
    "Yellow" : "Jaune",
    "Green" : "Vert",
    "Save threshold" : "Enregistrer le seuil",
    "Selection" : "Sélection",
    "Setting" : "Paramètre",
    "Title" : "Titre",
    "Delete data before load" : "Supprimer donnée avant le chargement",
    "No" : "Non",
    "Yes" : "Oui",
    "Execution" : "Exécuter",
    "Load now" : "Charger maintenant",
    "Test run" : "Lancement test",
    "Schedule in background" : "Planifiée en arrière-plan",
    "Not scheduled" : "Non planifié",
    "Daily" : "Quotidiennement",
    "Hourly" : "Toutes les heures",
    "Load via OCC command:" : "Charger via la commande OCC:",
    "Unshare" : "Ne plus partager",
    "can navigate" : "peut naviguer",
    "Password protection" : "Protection par mot de passe",
    "General information" : "Informations générales",
    "Report title" : "Titre du rapport",
    "Text variables can be used in the name or subheader. They are replaced when the report is executed." : "Des variables de texte peuvent être utilisées dans le nom ou le sous-en-tête. Ils sont remplacés lors de l'exécution du rapport.",
    "The following variables are available:" : "Les variables suivantes sont disponibles :",
    "Type of report" : "Type de rapport",
    "Realtime Data" : "Données temps-réel",
    "Stored Data" : "Données enregistrées",
    "Reports can read their data either from realtime data sources or from saved datasets within the Nextcloud database." : "Les rapports peuvent lire leurs données soit à partir de sources de données en temps réel, soit à partir de jeux de données enregistrés dans la base de données Nextcloud.",
    "Different data sources are available. The data is read in realtime when the report is being executed." : "Différentes sources de données sont disponibles. Les données sont lues en temps réel lors de l'exécution du rapport.",
    "Every datasource requires specific parameter. Please enter the information." : "Chaque source de données nécessite un paramètre spécifique. Veuillez saisir les informations.",
    "Existing dataset" : "Jeu de données existant",
    "The report can be based on a new dataset or read existing data that is available already." : "Le rapport peut être basé sur un nouveau jeu de données ou lire des données existantes déjà disponibles.",
    "Many reports can be based on the same data, but every report has its own visualization, threshold and sharing settings." : "Les rapports peuvent être basés sur les mêmes données, mais chaque rapport à son propre affichage, seuil et paramètres de partage.",
    "Datasets can store data from scheduled data loads, the REST API or manual data entry. This can be configured via the Dataset maintenance" : "Les jeux de données peuvent stocker des données à partir de chargements de données planifiés, de l'API REST ou de la saisie manuelle de données. Cela peut être configuré via la gestion du jeu de données",
    "Choose the names of the columns for the new dataset" : "Choisissez les noms des colonnes pour le nouveau jeu de données",
    "Visualization of the report" : "Visualisation du rapport",
    "All settings can be changed afterwards" : "Tous les paramètres peuvent être modifiés ultérieurement",
    "No Chart" : "Aucun Graphique",
    "No Table" : "Aucun Tableau",
    "The report can now be created" : "Le rapport peut maintenant être créé",
    "Create" : "Créer"
},
"nplurals=2; plural=(n > 1);");
