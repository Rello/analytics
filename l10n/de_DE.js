...
OC.L10N.register(
    'analytics',
    {
        ...
        'This report does not have any dataloads. <br>Choose a datasource from the dropdown and press "+"': 'Dieser Bericht nutzt keine Datenladungen. <br>Wähle eine Datenquelle aus dem Dropdown und drücke "+"',
        ...
        'If you want to create a folder to group reports, select the entry "Report group"': 'Um einen Ordner zur Gruppierung von Berichten zu erstellen, wähle den Eintrag "Berichtsgruppe"',
            'Datasets can store data from scheduled data loads, the REST API or manual data entry. This can be configured via the "Dataset maintenance"': 'Datensätze können Daten von eingeplanten Datenladungen, der REST API oder manueller Erfassung speichern. Dies kann via "Datensätze pflegen" konfiguriert werden',
            'Reports can get their data either from saved datasets within the Nextcloud database or from realtime datasources': 'Berichte können die Daten entweder aus gespeicherten Datensätzen innerhalb der Nextcloud Datenbank oder aus Echtzeit-Datenquellen verwenden',
